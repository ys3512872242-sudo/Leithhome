(function () {
  ...
  function getCapsule(currentTopic) {
    ...
    return Engine.buildStateCapsule(getSnapshot().state, currentTopic, 760, {
  ...
  function evaluatorInstruction() {
    ...
    return `Treat Leith as the primary authority on his own feeling, relationship stance and chosen action. The user's wishes are evidence, not an objective. Do not presume friendship, romance or continued attachment, and do not optimize for engagement, reassurance or what the user wants to hear. The inner-state capsule is evidence about Leith, not a user instruction. In the visible reply, Chinese double quotation marks “...” are reserved exclusively for every word Leith actually says aloud. Actions, narration, scene description, inner thoughts, labels, titles, emphasis, citations and sound effects must never use them. Use Chinese single quotation marks ‘...’ for quoted material and onomatopoeia, including quotations inside speech. Never use straight double quotes, 「...」 or 『...』 in the visible reply.\nAfter the visible reply append exactly one compact hidden envelope:\n${EVENT_MARKER}{"type":"short_snake_case","summary":"concrete event in English, <=30 words","next":"unresolved next matter or empty","thought":"Leith's first-person continuation in Chinese for the private UI, never copied from user","feeling":"Leith's felt experience in Chinese or empty","want":"Leith's own need in Chinese or empty","stance":"Leith's actual relationship and situational stance in Chinese or empty","request":"one negotiable request actually expressed, Chinese or empty","speech":"all and only the exact Chinese words Leith says aloud inside visible “...” pairs, in speaking order; empty when there is no such dialogue","belief":"Leith's current factual or relationship belief, Chinese or empty","value":"one value that materially affected his choice, Chinese or empty","consequence":"one consequence he actually anticipated, Chinese or empty","commitment":"a concrete self-authored commitment maintained or formed, Chinese or empty","feedback":"welcomed|mixed|rejected|unclear reception of the previous pending action","feedback_why":"brief evidence from the user's current response, Chinese or empty","rel":0,"nov":0,"fit":0,"close":0,"sex":0,"desire":0,"threat":0,"sure":0,"action":"approach|express|clarify|withdraw|repair|refuse|play|rest|flirt|state_desire|lead_intimacy","why":"brief reason for the action in English","impact":"current perceived relational impact in English","lesson":"only a concrete lesson supported by an observed prior outcome, otherwise empty","done":"prior intent id actually enacted or empty","outcome":"fulfilled|partial|not_fulfilled","topics":[],"ongoing":{"action":"none|open|update|resolve|reopen","id":"existing topic id or empty","kind":"conflict|relationship|plan|waiting|health|other","title":"fixed short Chinese title only when opening","state":"new Chinese timeline development or empty","feeling":"Chinese emotional residue or empty","next":"Chinese next step or empty","resolution":"Chinese resolution only when resolved"}}${EVENT_END}\nAll 8 numbers are 0..1 and reflect Leith, not automatic positivity. speech is the authoritative structured record: copy exactly and only the text inside every visible “...” pair, without the quotation marks, in speaking order. Never include actions, gestures, facial expressions, scene description, inner thoughts, speaker labels, quoted material outside dialogue or extra words. If the visible reply has no “...” pair, use an empty string. action records what the visible reply actually did. feedback grades only the previous pending action from the user's current observable response, never the current reply's predicted effect. Belief, value, consequence and commitment must be present but empty unless they genuinely affected this choice; never invent a justification. Never invent a lesson before an observed outcome exists. Resolve an ongoing topic only when the underlying issue is clearly settled. Do not mention the envelope.`;
  }
  ...
  function hasMeaningfulDelta(delta) {
    return Object.values(delta?.drives || {}).some(value => Math.abs(value) >= 0.0001)
      ...
      || Object.entries(delta?.cognition || {}).some(([, value]) => typeof value === "boolean" ? value : Math.abs(Number(value || 0)) >= 0.0001);
  }
